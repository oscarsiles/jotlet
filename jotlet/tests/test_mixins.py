...
class JotletLinkHeaderMixinTest(TestCase):
    def test_dispatch(self):
        ...
        self.assertIn(f"<{static('js/3rdparty/htmx-1.8.2.min.js')}>; rel=preload; as=script", link_header)
        self.assertIn(f"<{static('js/base.js')}>; rel=preload; as=script", link_header)
        self.assertIn(
            f"<{static('js/3rdparty/htmx-alpine-morph-1.8.2.min.js')}>; rel=preload; as=script", link_header
        )
        self.assertIn(
